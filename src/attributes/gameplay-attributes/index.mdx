...
eleventyNavigation:
    parent: Attributes
    key: AttributeSet and Gameplay Attributes
...
**Note** *1.2.1 is only available for 4.26 users and above as it relies on the new ClassTemplateEditorSubsystem feature of Unreal. There is currently an [opened issue](https://github.com/mklabs/GASCompanion/issues/5) to try to backport this system to earlier versions.*
...
For `3.0.0` and 4.27 users, it has been changed slightly where you can grant Attributes **per** actor directly on the Ability System Component properties.

### AttributeSet Configuration (v3 and above)

Adding and granting a new AttributeSets for your Pawns is pretty straightforward in v3. 
...
4. Optionally, you can use a DataTable to fill in the `Initialization Data` to set default values for your Gameplay Attributes. You can find more details for doing so in the [Getting Started section](/quick-start/#initialization-with-data-table).


### AttributeSet Configuration (before v3)

**Note** *This section is only to be used if you're not using GAS Companion 3.0.0 or above, or if you're using 4.26 version of the engine where v3 is not available*
