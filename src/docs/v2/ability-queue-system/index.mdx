...
Hit `²` while the game is running to open the Console Command (more info in the [Cheat Manager](/cheat-manager) page), and type `GSC_AbilityQueueDebug`. It will display the debug widget from where you can see the state of the Ability Queue (enabled, opened, allowed abilities, currently queued ability and montages)
