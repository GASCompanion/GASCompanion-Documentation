...
You can change the HUD User Widget with the [plugin configuration](/configuration) available in the Project's Settings.
...
Those are all the `meta=(BindWidget)` UPROPERTY `UGSCUWHud` is using to update the UI. You just need to create the appropriate Widget with the exact same name in your UserWidget hierarchy.
