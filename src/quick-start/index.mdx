...
**Note** If you're looking for the previous Quick Start documentation for 4.26, you can find it [here](/v2/quick-start)
...
`GSCAbilitySystemComponent` is a child of `AbilitySystemComponent` from GAS, and automatically created for you in `ModularCharacter` and `ModularPawn` (as well as `ModularPlayerState` if you wish to have ASC living on PlayerState)
